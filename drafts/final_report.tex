...
Apart from that the storage module was added, providing abstract functions for storing and retrieving the data each peer has to handle.
It also provides a simple implementation of a notification system, enabling functionality similar to Java's \texttt{Thread.notify} since Go's own functions for synchronization did not fully provide the required functionality.
...
\begin{table}
    ...
    \begin{tabular}{c|p{.75\textwidth}}
        ...
        \texttt{MSG\_KEYXCHGRESP} & May only be sent in response to a \texttt{MSG\_KEYXCHG} and contains the response to the Diffie-Hellman key exchange. \\
        \texttt{MSG\_EXTEND} & After the key exchange was successful, this message indicates to a peer that the tunnel should be extended to another peer. The message contains the next peer's IP address, onion port and the encrypted Diffie-Hellman nonce for the key exchange between the tunnel initiator and the next peer. When this message is received, the message type is replaced by \texttt{MSG\_KEYXCHG} and the nonce is sent along to the new peer. It is then expected that the new peer answers with \texttt{MSG\_KEYXCHGRESP} which is forwarded backwards through the tunnel to the initiator which can then derive the shared secret with this new hop. \\
        \texttt{MSG\_COMPLETE} & As an alternative to \texttt{MSG\_EXTEND} the initiator can also send \texttt{MSG\_COMPLETE} through the tunnel to a hop with which it just completed the Diffie-Hellman key exchange. This indicates to this hop that it is the destination of the tunnel and that it can now send and receive data through it. \\
        \texttt{MSG\_COMPLETED} & Send by a peer who received a \texttt{MSG\_COMPLETE} message. When the initiator receives the \texttt{MSG\_COMPLETED} message it know that the tunnel has been built successfully and that data can be sent through it in both directions. \\
    \end{tabular}
...
\begin{table}
    ...
    \begin{tabular}{c|p{.75\textwidth}}
        ...
        \texttt{MSG\_KEEPALIVERESP} & May only be sent to the initiator of a tunnel. Indicates to the initiator that the tunnel is working and passing data correctly. \\
        \texttt{MSG\_COVER} & May only be sent to the tunnel destination. This message contains random data which is expected to be returned to create illusion of an actively used tunnel. \\
...
This process is also shown for two intermediate peers in Figure \ref{fig:tunnel-build}.
API communication and other parts of the module that are not directly involved in the module's onion communication have been left out to simplify the depiction.
\begin{figure}
    \centering
    \includegraphics[scale=0.65]{onion-tunnel-simplified.pdf}
    \caption{Simplified depiction of the tunnel build process with two intermediate peers}
    \label{fig:tunnel-build}
\end{figure}

\subsection{Timeouts and Keepalives}
To ensure that a tunnel that stopped working does not use resources on the peers unnecessarily each peer monitors all their active tunnels and removes the tunnel if no data has been received for a set amount of time (the variable \texttt{TUNNEL\_INACTIVITY\_TIMEOUT} is set to five seconds in the current implementation).
...
This reduces the number of message required to be sent when a tunnel is destroyed and it is not expected that the short time span where the tunnel still exists on the intermediate peers would cause any performance impact.
...
A guide containing specific commands to achieve each step can be found in the projects Readme file, under \texttt{implementation/src/README.md}.
...
\end{enumerate}
When the parameter for the configuration file path is not given the binary searches for a file called \texttt{config.ini} in the current folder and tries to use it as its configuration.

\subsection{Testing}
To simplify testing the implementation, a testing harness was created which enables simulating a multi-peer setup on a single machine.
...
\begin{itemize}
    ...
    \item \texttt{TestCoverTraffic} sends a \texttt{ONION\_COVER} command to the first peer's API which then creates a tunnel and sends cover traffic through it
\end{itemize}
...
Firstly, the system does not make any guarantees for the reliability of the data delivery, all message are sent on a best-effort basis.
...
Apart from that all packets are fixed to a size of 1232 bytes and for each hop there is an additional overhead of 21 bytes per packet (16 bytes for cryptography data, 4 bytes sequence number and 1 byte message type).
...
Lastly, increasing the number of intermediate hops will decrease the amount of data that can be sent per packet and the reliability will decrease due to the nature of sending more UDP packets that might be lost in transmission.

\section{Future work}
In the future the system could be improved mainly with regards to security, reliability of the data delivery and configurability.
...
To improve the reliability of the data stream a TCP-like layer could be introduced over the top of the Onion layer, ensuring that lost packets are resent and that the messages are delivered to the API order.
While this was explicitly not required by the specification it would still greatly improve the applicability of this implementation to general purpose programs.

Lastly, some parameters of the program are still defined as constants within the code.
...
All in all, these changes can generally be considered as improvements, but they also come with downsides that potentially make them less desirable.

\section{Workload distribution}
\todo{Create this section}
